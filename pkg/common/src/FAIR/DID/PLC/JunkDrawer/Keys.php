...
class Keys {
     ...
     */
    public static function generate_keypair() : KeyPair {
        $ec = new EC( self::CURVE_K256 );
     ...
     */
    public static function decode_public_key( string $key ) : KeyPair {
        static $cache = [];
        if ( isset( $cache[ $key ] ) ) {
            return $cache[ $key ];
        }
     ...
     * @return KeyPair The keypair object.
     */
    public static function decode_private_key( string $key ) : KeyPair {
        static $cache = [];
        if ( isset( $cache[ $key ] ) ) {
            return $cache[ $key ];
        }
     ...
     */
    public static function encode_public_key( KeyPair $key, string $curve ) : string {
        $pub = $key->getPublic( true, 'hex' );
        $prefix = match ( $curve ) {
            self::CURVE_K256 => 'e701',
            self::CURVE_P256 => '8024',
            default => throw new Exception( 'Unsupported curve' ),
        };
     ...
     */
    public static function encode_private_key( KeyPair $key, string $curve ) : string {
     ...
     */
    public static function encode_did_key( KeyPair $key, string $curve ) : string {
        $encoded = self::encode_public_key( $key, $curve );
