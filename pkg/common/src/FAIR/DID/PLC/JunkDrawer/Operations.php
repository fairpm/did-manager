...
use Elliptic\Utils;
use FAIR\DID\PLC\CanonicalMapObject;
...
class Operations
{
     ...
     */
    public static function sign_operation(Operation $data, KeyPair $signing_key): SignedOperation
    {
        // Validate the operation.
        $data->validate();

        // Encode the operation into DAG-CBOR.
        $encoded = self::encode_operation($data);
     ...
     */
    public static function genesis_to_plc(SignedOperation $data): string
    {
        $encoded = self::encode_operation($data);
        $hash = hash('sha256', $encoded, true);
        $encoded = Encode::base32_encode($hash);
     ...
     */
    public static function encode_operation(Operation $data): string
    {
        $operation = CanonicalMapObject::create()
            ...
            ->add(
                ...
                ListObject::create(array_map(fn(KeyPair $key) => TextStringObject::create(Keys::encode_did_key($key,
                    Keys::CURVE_K256)), $data->rotationKeys)),
            )
            ->add(
     ...
     */
    public static function cid_for_operation(SignedOperation $op): string
    {
        $cbor = self::encode_operation($op);
